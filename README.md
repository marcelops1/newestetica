...
- **Qualidade**: 14 arquivos e 104 testes a 100% de cobertura; gates (`lint`, `format`, `typecheck`, `test`, `build`) em 3 camadas — pre-commit local (husky + lint-staged), CI (`.github/workflows/quality.yml` com auditoria e Gitleaks) e branch protection exigindo o check "quality gates".
- **OpenSpec**: 7 specs de domínio (72 requirements) e 23 changes arquivados (22 com specs sincronizadas); 19 PRs mergeados em `main`.
