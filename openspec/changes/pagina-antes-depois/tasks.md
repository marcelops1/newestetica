...
- [ ] 1.1 RED: escrever `frontend/lib/__tests__/before-after.test.ts` importando `getBeforeAfterPageCases` de `../before-after` e asserindo que `resultado-3` (sem consentimento) nunca aparece e que todo caso listado tem `hasConsent` e painel completo, e verificar que a suite falha (módulo ainda inexistente)
