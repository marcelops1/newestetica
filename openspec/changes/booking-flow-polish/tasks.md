...
- [ ] 2.1 Escrever a verificação dos quatro estados (formulário, enviando com duplo clique ignorado, sucesso com resumo, erro com nova tentativa preservando dados) e constatar que ela falha com o modal atual
- [ ] 2.2 Implementar os quatro estados no modal e verificar que a verificação da task 2.1 passa
- [ ] 2.3 Escrever os casos de verificação da validação amigável (nome vazio e WhatsApp inválido orientam a correção via `aria-invalid`/`aria-describedby`) e constatar que falham com o modal atual
...
- [ ] 3.1 Escrever a verificação de teclado do focus trap (Tab circula no diálogo, Escape fecha devolvendo o foco) e constatar que ela falha com o modal atual
- [ ] 3.2 Implementar o focus trap mantendo Escape, overlay e devolução de foco, e verificar que a verificação da task 3.1 passa
