...
- [x] 2.2 Extrair `TreatmentCard` + `CategoryFilter` reutilizáveis sem mudar a home e verificar que a home renderiza idêntica
- [x] 2.3 Implementar `/tratamentos` (lista + filtro + busca) e `/tratamentos/[slug]` (detalhe + 404 acolhedora + CTA via `BookingModal`) e verificar que os testes da task 2.1 passam
