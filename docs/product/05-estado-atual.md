...
- **Dívida técnica (retenção de PII em backups — Pacientes):** política de retenção/expurgo de PII em backups do banco (paciente é anonimizado pela aplicação — placeholders + `status`/`anonymizedAt` —, mas backups pré-anonimização retêm PII) — avaliar quando existir estratégia real de backup em produção. O requisito correlato de que módulos futuros com join em Pacientes respeitem o filtro `findVisible` foi **resolvido pelo change `backend-modulo-atendimento`** (primeiro join: o módulo Atendimento/Histórico respeita a visibilidade herdada em todos os pontos de leitura — porta `PatientDirectory` + filtro de relação na query —, provado por write-then-throw em duas camadas). Registro original: revisão final do PR #42 (R3) e `verification.md` do change arquivado `2026-09-25-backend-modulo-pacientes` (§8).
